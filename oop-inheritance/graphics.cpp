...
rgbcolor window::border = rgbcolor{255, 0, 0};
...
void window::display() {
   ...
   //set polygonmode back to GL_FILL to draw the whole polygon
   glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
   for (auto& object : window::objects) object.draw();
   glLineWidth(window::thickness);
   //after drawing the polygon, set mode to GL_LINE to draw border
   glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
   window::objects[selected_obj].drawborder(window::border);
   mus.draw();
   glutSwapBuffers();
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   case 'N': case 'n': case SPACE: case TAB:
      select_object_((selected_obj == objects.size() - 1) ?
                     0 : (selected_obj + 1));
      break;
   case 'P': case 'p': case BS:
      select_object_((selected_obj == 0) ? (objects.size() - 1) :
                     (selected_obj - 1));
      break;
   case '0'...'9':
      select_object_(key - '0');
      break;
   default:
...
void window::select_object_ (size_t obj) {
   if (objects.size() <= obj) {
   } else {
      selected_obj = obj;
   }
}
void window::move_selected_object (float x, float y) {
   if (selected_obj < objects.size()) {
      objects[selected_obj].move(x * move_by, y * move_by);
   }
}
