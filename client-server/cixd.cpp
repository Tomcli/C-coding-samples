...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if (file == NULL) {
      ...
      log << "get file " << header.filename << " failed: "
          << strerror(errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet(client_sock, &header, sizeof header);
...
void reply_rm (accepted_socket& client_sock, cix_header& header) {
   ...
   if (rc) {
      //if unlink file failed, return error message
      log << "remove file " << header.filename << " failed: "
          << strerror(errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet(client_sock, &header, sizeof header);
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   ...
   ofstream file (header.filename, ofstream::out);
   recv_packet(client_sock, &buffer, sizeof buffer);
   //received the buffer and write on the server file 
   log << "received " << sizeof buffer << " bytes" << endl;
